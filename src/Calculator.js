...
function Calculator() {
  ...
  const removeHistoryItem = (id) => {
    const newHistorys = history.filter((item) => item.id !== id);
  ...
  const handleRemoveAllHistory = () => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        localStorage.setItem("old_list_history", JSON.stringify(history));
        setSelectedSum(0);
        setHistory([]);
      }
    });
  ...
  const restoreOldHistory = () => {
    const old = JSON.parse(localStorage.getItem("old_list_history")) ?? [];

    if (old.length === 0) {
      return Swal.fire({
        icon: "error",
        title: "Empty old history!",
    ...
    Swal.fire({
      title: "Are you sure?",
      text: "This action will replace the current history!",
      icon: "question",
      showCancelButton: true,
      confirmButtonColor: "#ff9f0a",
      cancelButtonColor: "#555",
        ...
        {isOpenHistory ? (
          <div className="header-actions">
            <button
              ...
              className="btn close-history-btn btn-color-orange"
              title="Go back calculator"
            >
              <MdOutlineCalculate />
            </button>
            <button
              onClick={handleSelectedAll}
              className="clear-btn btn btn-color-orange"
              title="Select or Unselect all history"
            >
              {checkedAll ? (
                <MdOutlineCheckBox />
              ) : (
            ...
            <button
              onClick={restoreOldHistory}
              className="restore-btn btn btn-color-orange"
      ...
      <>
        {isOpenHistory ? (
          ...
          <div className="calculator-wrapper">
            ...
            >
              <button
                className="btn open-history-btn"
                style={{
                  fontSize: "2rem",
